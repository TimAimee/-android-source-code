...
public class CurlMesh {
	...
	private static final double BACKFACE_ALPHA = 1.0f;//
	...
	private boolean isLeftBackShow = false;
	...
	public synchronized void draw(GL10 gl) {
		...
		if (DRAW_TEXTURE && mBitmap != null) {// 纹理贴图
			gl.glBindTexture(GL10.GL_TEXTURE_2D, mTextureIds[0]);
		...
		// //贴背景
		if (isLeftBackShow) {
			if (DRAW_TEXTURE && mLastBitmap != null) {
				gl.glBindTexture(GL10.GL_TEXTURE_2D, mTextureIds[1]);
				GLUtils.texImage2D(GL10.GL_TEXTURE_2D, 0, mLastBitmap, 0);
				//mLastBitmap.recycle();
				mLastBitmap = null;
			...
			if (DRAW_TEXTURE && mNextBitmap != null) {
				gl.glBindTexture(GL10.GL_TEXTURE_2D, mTextureIds[1]);
				GLUtils.texImage2D(GL10.GL_TEXTURE_2D, 0, mNextBitmap, 0);
					mNextBitmap.recycle();
					mNextBitmap = null;
		...
		if (DRAW_TEXTURE) {
			...
			gl.glDrawArrays(GL10.GL_TRIANGLE_STRIP, backStartIdx, backCount);// 贴图
	...
	/**
	 * Sets Last texture for this mesh.
	 */
	public synchronized void setLastPageBitmap(Bitmap bitmap) {
		...
		if (DRAW_TEXTURE) {
			...
			mLastBitmap = Bitmap.createBitmap(newW, newH, bitmap.getConfig());
			Canvas c = new Canvas(mLastBitmap);
			Matrix mx=new Matrix();
			mx.setScale(-1, 1,bitmap.getWidth()/2,bitmap.getHeight()/2);
	...
	/**
	 * Sets Next texture for this mesh.
	 */
	public synchronized void setNextPageBitmap(Bitmap bitmap) {

		isLeftBackShow = false;
		...
		if (DRAW_TEXTURE) {
			...
			// TODO: Is there another way to create a bigger Bitmap and copy
			// original Bitmap to it more efficiently? Immutable bitmap anyone?
			mNextBitmap = Bitmap.createBitmap(newW, newH, bitmap.getConfig());
			Canvas c = new Canvas(mNextBitmap);
	...
	public synchronized void setFlipTexture(boolean flipTexture) {
		...
		// setTexCoords(mTextureRect.left, mTextureRect.top, mTextureRect.right,
		// mTextureRect.bottom);
		if (mFlipTexture) {
			setTexCoords(mTextureRect.right, mTextureRect.top,
					mTextureRect.left, mTextureRect.bottom);
